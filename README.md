...
> **银行合规风控 AI SKILL 集** — 天眼查 OpenAPI + MCP 协议驱动的 9 个银行向业务 SKILL
...
|    | /tyc-enterprise-report|      ✅ UBO 多层股权穿透          |
|    +----------------------+                                   |
|                                                               |
+---------------------------------------------------------------+
```
...
| 9 | `/tyc-enterprise-report` | 企业全景报告 | ~2min | 客户经理述职、年度客户回顾 |
...
所有 SKILL 通过单一 MCP server `tyc`（URL: `https://ai-mcp.tianyancha.com/mcp`）调用业务语义聚合层共 **167 个**聚合工具，分 6 大分类：

| Go 包 | 中文分类 | 工具数 | 本仓主要使用 |
|-------|---------|--------|------------|
| `company` | 企业基础信息 | 52 | kyb / credit-dd / ubo / guarantor |
| `risk` | 风险合规 | 36 | aml / kyb / counterparty / credit-monitor |
| `executive` | 董监高 | 15 | aml（PEP 识别）/ ubo |
| `operation` | 经营与公示 | 32 | trade-compliance |
| `intellectual_property` | 知识产权 | 14 | enterprise-report |
...
| MCP endpoint | `https://ai-mcp.tianyancha.com/mcp` | 公网托管，无需自部署 |
